...
class PestaticSignatureParser:
    CANDIDATE_STRIDES = (16, 12, 20, 24)
    TRY_HEADER_SIZES = (4, 2, 0)
    ACCEPTED_TAIL_PADS = (0, 4, 8)
    MAX_RECORDS = 1_000_000

    @classmethod
    def _try_grid(cls, content, header_size, stride):
        if header_size not in (0, 2, 4):
            return None
        if len(content) < header_size + stride:
            return None
        ...
        if header_size == 2:
            ...
            pads = [
                pad
                for pad in cls.ACCEPTED_TAIL_PADS
                if grid_start + count * stride + pad == len(content)
            ]
            if not pads:
                return None
            pad = pads[0]
        ...
        for _ in range(count):
            if pos + stride > len(content):
                return None
            entries.append(content[pos:pos + stride])
            pos += stride
    ...
    def parse(cls, content: bytes):
        for header in cls.TRY_HEADER_SIZES:
            for stride in cls.CANDIDATE_STRIDES:
                result = cls._try_grid(content, header, stride)
                if result is None:
                    continue
                ...
                for i, entry in enumerate(result["entries"]):
                    line = (
                        f"    Entry[{i}]: "
                        f"{_ProjectParserUtils.hex_spaced(entry)}"
                    )
                    if len(entry) >= 16:
                        d = struct.unpack_from("<IIII", entry, 0)
                        line += (
                            f" | d0=0x{d[0]:08X} d1=0x{d[1]:08X} "
                            f"d2=0x{d[2]:08X} d3=0x{d[3]:08X}"
                        )
                    hints = []
                    if len(entry) >= 4:
                        x0 = int.from_bytes(entry[:4], "little")
                        if x0 in (0, 0xFFFFFFFF):
                            hints.append("crc-ish degenerate")
                    if len(entry) >= 8:
                        x1 = int.from_bytes(entry[4:8], "little")
                        if x1 < 0x01000000:
                            hints.append("index-ish small")
                    if len(entry) >= 12:
                        x2 = int.from_bytes(entry[8:12], "little")
                        if 0 < x2 < 256 * 1024 * 1024:
                            hints.append("size-ish")
                    if hints:
                        line += " | " + ", ".join(hints)
                    lines.append(line)
                return lines
...
class DefenderSignatureStreamParser:
    def __init__(self, db_path="Threats.db", weight_log_path=WEIGHT_ANOMALY_LOG, weight_anomaly_ratio=WEIGHT_ANOMALY_RATIO):
        ...
        for _text_type in (
            0x29, 0x49, 0xDC, 0xAD, 0x6A, 0x98, 0x4A, 0x43,
            0x64, 0x6C, 0xD0, 0xA8,
        ):
